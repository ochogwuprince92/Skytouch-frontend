...
export function PlanCard({ plan, isRecommended, onSubscribe, formatPrice, billingCycle }: PlanCardProps) {
  ...
  const features = [
    ...
    plan.plan !== 'BASIC' && plan.plan !== 'FREE' && 'Candidate search',
  ...
  return (
    <div
      className={`relative bg-white rounded-2xl shadow-sm p-8 transition-all duration-300 hover:shadow-lg hover:-translate-y-1 ${
        isRecommended ? 'border-2 border-primary ring-4 ring-primary/10' : 
        isFree ? 'border-2 border-green-500 ring-4 ring-green-500/10' :
    ...
    >
      ...
      <div className="text-center mb-6">
        ...
        <div className="text-4xl font-bold text-slate-900 mb-1">
          {isFree ? 'Free' : formatPrice(plan.price, billingCycle)}
        </div>
        {!isFree && <p className="text-slate-500 text-sm">per {billingCycle.toLowerCase()}</p>}
      </div>
      ...
      {isFree ? (
        <button
          disabled
          className="w-full py-3 px-6 rounded-xl font-semibold bg-slate-100 text-slate-400 cursor-not-allowed"
        >
          Current Plan
        </button>
      ) : (
